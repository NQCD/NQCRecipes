...
:::{.callout-note}
This tutorial focuses using [NQCModels.jl](https://github.com/NQCD/NQCModels.jl) to build a Newns-Anderson model for a molecule chemisorped to a surface. In the process we will also see how one can use [NQCCalculators.jl](https://github.com/NQCD/NQCCalculators.jl) to evaulate and update important properties of the model.
:::

## Dependencies
The follwing packages are all that is required to complete this tutorial. If you don't want to add them to your default Julia environment simply type `] activate 'NQCD Newns-Anderson tutorial'` into the REPL before adding the packages
```julia
